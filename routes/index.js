...
router.post('/new2/order', function(req, res, next) {
  console.log('DEPRECATED: POST /new2/order — use POST /new/order');
  return res.status(410).send('deprecated use /new/order');
});
...
router.get('/order/reprint/pdf/:id', isLoggedIn, function(req, res, next) {
  ...
  ordersDB.findOne({
    ...
    if (err) {
      console.log(err);
      return res.send('THERE WAS AN ISSUE PRINTING, LET TREY KNOW IMMEDIATELY');
    }
