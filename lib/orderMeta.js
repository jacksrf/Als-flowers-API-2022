...
// All print_status values that mean "the order did not make it onto paper and a
// human needs to retry". Kept in one place so views, filters, and reconcilers
// stay in sync.
var FAILED_PRINT_STATUSES = [
  'error',
  'expired',
  'admin_error',
  'admin_unreachable',
...
function isFailedPrintStatus(status) {
  if (!status) {
    return false;
  }
  return FAILED_PRINT_STATUSES.indexOf(status) !== -1;
}

var PRINT_STATUS_LABELS = {
  error: 'Print failed',
  expired: 'Print expired',
  admin_error: 'Admin page error',
  admin_unreachable: 'Admin unreachable',
  webshot_error: 'Screenshot failed'
};
...
function enrichOrderForList(order, db) {
  ...
  if (order.printed_at && !isFailedPrintStatus(order.print_status)) {
  ...
  } else if (isFailedPrintStatus(order.print_status)) {
    var failureLabel = PRINT_STATUS_LABELS[order.print_status] || 'Print failed';
    var failureDetail = order.print_error || order.print_status;
    order.printedLabel = failureLabel;
    order.printedTitle = failureDetail + (order.printnode_job_id ? ' (job #' + order.printnode_job_id + ')' : '');
...
function applyListFilter(orders, filter) {
  ...
  if (filter === 'print_failed') {
    return orders.filter(function(o) { return o.isPrintFailed === true; });
  }
  return orders;
}
...
function persistOrderMetaOnInsert(db, doc, callback) {
  ...
  if (!db || typeof db.get !== 'function') {
    console.log('persistOrderMetaOnInsert: db handle missing (order #' + (doc && doc.order_number) + ') — skipping meta write');
    return cb();
  }
  if (!doc || !doc._id) {
    console.log('persistOrderMetaOnInsert: doc missing _id — skipping meta write');
    return cb();
  }
  ...
  ordersDB.update({ _id: doc._id }, { $set: updates }, cb);
}
...
module.exports = {
  ...
  FAILED_PRINT_STATUSES: FAILED_PRINT_STATUSES,
  PRINT_STATUS_LABELS: PRINT_STATUS_LABELS,
  isFailedPrintStatus: isFailedPrintStatus,
