...
var ADMIN_PREFLIGHT_TIMEOUT_MS = 15000;

// Heroku's generic crash page ("Application error", H10/H12/H13, etc.) and a
// few other common upstream-error fingerprints. If any of these appear we
// refuse to send the page to the printer.
var ADMIN_ERROR_PATTERNS = [
  /<title>\s*Application Error\s*<\/title>/i,
  /no-such-app\.html/i,
  /heroku-error/i,
  /<title>\s*Heroku\s*\|\s*Application Error\s*<\/title>/i,
  /An error occurred in the application and your page could not be served/i,
  /<title>\s*502 Bad Gateway\s*<\/title>/i,
  /<title>\s*503 Service Unavailable\s*<\/title>/i,
  /<title>\s*504 Gateway Time-?out\s*<\/title>/i
];
...
function detectAdminErrorBody(body) {
  if (!body || typeof body !== 'string') {
    return null;
  }
  for (var i = 0; i < ADMIN_ERROR_PATTERNS.length; i++) {
    if (ADMIN_ERROR_PATTERNS[i].test(body)) {
      return ADMIN_ERROR_PATTERNS[i].source;
    }
  }
  return null;
}

function preflightAdminPage(adminUrl, done) {
  request.get({
    url: adminUrl,
    timeout: ADMIN_PREFLIGHT_TIMEOUT_MS,
    followRedirect: true,
    headers: { 'User-Agent': 'als-flowers-print-preflight/1.0' }
  }, function(err, response, body) {
    if (err) {
      return done({
        status: 'admin_unreachable',
        message: 'Admin pre-flight network error: ' + err.message
      });
    }
    ...
    if (!statusCode || statusCode < 200 || statusCode >= 300) {
      return done({
        status: 'admin_error',
        message: 'Admin returned HTTP ' + statusCode + ' for ' + adminUrl
      });
    }

    var match = detectAdminErrorBody(body);
    if (match) {
      return done({
        status: 'admin_error',
        message: 'Admin page matched upstream error pattern (' + match + ') for ' + adminUrl
      });
    }
...
function runPrintJob(doc, db, options, done) {
  ...
  preflightAdminPage(adminUrl, function(preflightErr) {
    if (preflightErr) {
      console.log('SKIP PRINT #' + doc.order_number + ': ' + preflightErr.message);
      return recordPrintFailed(db, doc, null, preflightErr.status, preflightErr.message, function() {
        if (done) {
          done(new Error(preflightErr.message));
        }
      });
    }
    ...
    webshot(adminUrl, pdfPath, getWebshotOptions(), function(err) {
      if (err) {
        console.log('webshot error for order #' + doc.order_number + ':', err);
        recordPrintFailed(db, doc, null, 'webshot_error', err.message || String(err), function() {
          if (done) {
            done(err);
          }
        });
...
function finishWebhookWithPrint(doc, db, res, nl2br) {
  ...
  if (!db || typeof db.get !== 'function') {
    console.log('finishWebhookWithPrint: db handle missing for order #' + doc.order_number + ' — acking webhook without persist/print');
    prepareOrderDoc(doc, nl2br);
    webhookAck(res);
    return;
  }
  ...
  orderMeta.persistOrderMetaOnInsert(db, doc, function() {
    ...
    if (shouldPrint(doc, nl2br) && db && typeof db.get === 'function') {
...
module.exports = {
  ...
  detectAdminErrorBody: detectAdminErrorBody,
  preflightAdminPage: preflightAdminPage
};
